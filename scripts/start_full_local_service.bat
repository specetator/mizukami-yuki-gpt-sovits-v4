...
echo Waiting before starting the local emotion adapter...
timeout /t 8 /nobreak >nul

echo Starting local emotion adapter on http://127.0.0.1:%YUKI_ADAPTER_PORT%/docs
"%GPT_SOVITS_ROOT%\runtime\python.exe" "%WORKFLOW_ROOT%\scripts\emotion_tts_adapter.py" ^
  --refs "%WORKFLOW_ROOT%\refs\emotion_refs.json" ^
  --gpt-sovits-api "http://127.0.0.1:%YUKI_INNER_API_PORT%" ^
  --host 0.0.0.0 ^
  --port %YUKI_ADAPTER_PORT%

echo.
echo Local emotion adapter exited with code %ERRORLEVEL%.
pause
